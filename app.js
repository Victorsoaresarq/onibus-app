...
const CONFIG = {
  ...
  // Chave gratuita da CARTO para remoção de marca d'água
  CARTO_API_KEY: 'cb1_32sn_1_4d7410698d416ef9b9612ed8',
...
// CartoDB Voyager: estilo claro, minimalista e elegante (estilo Apple Maps)
const cartoKeyParam = CONFIG.CARTO_API_KEY ? `?key=${CONFIG.CARTO_API_KEY}` : '';
...
function mudarAbaTutorial(so) {
  const isIos = so === 'ios';
  document.getElementById('tabBtnIos')?.classList.toggle('active', isIos);
  document.getElementById('tabBtnAndroid')?.classList.toggle('active', !isIos);
  document.getElementById('tabContentIos')?.classList.toggle('active', isIos);
  document.getElementById('tabContentAndroid')?.classList.toggle('active', !isIos);
}
...
function fecharModalSeClicarFora(event, modalId) {
...
function enviarFeedback(e) {
  e.preventDefault();
  const linha = document.getElementById('feedbackLinha')?.value.trim() || 'Geral';
  const texto = document.getElementById('feedbackTexto')?.value.trim() || '';

  if (!texto) return;

  const assunto = encodeURIComponent(`[Cadê o Ônibus] Relato Linha ${linha}`);
  const corpo = encodeURIComponent(`Linha: ${linha}\n\nMensagem/Relato:\n${texto}\n\nDispositivo: ${navigator.userAgent}\nHorário: ${new Date().toLocaleString('pt-BR')}`);

  window.location.href = `mailto:contato.mobilidade.rio@gmail.com?subject=${assunto}&body=${corpo}`;

  fecharModalContato();
  mostrarStatus('✅ Abrindo cliente de e-mail para enviar seu relato...');
}
